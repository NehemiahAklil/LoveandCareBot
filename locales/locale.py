...
class Locale:
    ...
    def reload_strings(self):
        for filename in os.listdir(r"./locales"):
            if filename.endswith(".yaml"):
                language_name = filename[:-5]
                self.languages[language_name] = yaml.safe_load(
                    open(r"./locales/" + filename, encoding="utf8"))

    def new_strings(self, filename):
        try:
            new_language = yaml.safe_load(open(r"./locales/" + filename))
                    ...
                    if pop_string:
                        for language in self.languages:
                            if not language == "en":
                                ...
                                with open(r"./locales/" + language + ".yaml", 'w') as outfile:
        ...
        if missing_arguments:
            with open(r"./locales/" + filename, 'w') as outfile:
